...
==============================================================
物体認識は通常 3 クラス以上を扱うため、二値分類器を
One-vs-Rest で束ねるか、softmax によるネイティブ多クラス
分類器を使う。

選択アルゴリズム:
  1. KNNClassifier        - 学習不要、コサイン距離
  2. SoftmaxRegression    - 多項ロジスティック回帰 (softmax)
  3. OvRLinearSVM         - 線形 SVM の One-vs-Rest 拡張
  4. MLPClassifier        - 2 層 MLP + softmax (非線形性)

精度を最優先するための工夫:
  - SoftmaxRegression: log-sum-exp による数値安定化
  - SVM: Pegasos 風の平均化 SGD で汎化性能を向上
  - MLP: He 初期化 + Adam + 早期停止
"""
